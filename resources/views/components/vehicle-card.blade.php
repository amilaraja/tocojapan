...
<a href="{{ route('vehicles.show', $vehicle->slug) }}" class="group block bg-white border border-line hover:border-toco-navy transition relative">
    <div class="relative aspect-[4/3] bg-toco-silver-2 overflow-hidden">
        ...
        @endif
        <button type="button" class="absolute top-2 right-2 w-8 h-8 grid place-items-center rounded-full bg-white/90 border border-line hover:border-toco-red text-ink-soft hover:text-toco-red" aria-label="Save vehicle" onclick="event.preventDefault();event.stopPropagation();">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21s-7-4.5-7-10a4 4 0 0 1 7-2.7A4 4 0 0 1 19 11c0 5.5-7 10-7 10Z"/></svg>
        </button>
    </div>
    <div class="p-3">
