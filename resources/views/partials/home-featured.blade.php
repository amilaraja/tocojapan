            ...
            <aside class="bg-white border border-line rounded-sm">
                ...
                <ul class="text-[13px]">
                    @foreach ($makesWithCounts as $make)
                        <li>
                            <a href="{{ route('vehicles.index') }}?make={{ $make->slug }}" class="flex items-center justify-between px-3 py-2 hover:bg-toco-silver-2 border-b border-line/60 last:border-b-0">
                                <span class="font-semibold">{{ $make->name }}</span>
                    ...
                    @foreach ($bodyTypesWithCounts as $bt)
                        <li>
                            <a href="{{ route('vehicles.index') }}?body_type={{ $bt->slug }}" class="flex items-center gap-2 px-3 py-2 hover:bg-toco-silver-2 border-b border-line/60 last:border-b-0">
                                <span class="w-7 h-7 grid place-items-center text-toco-navy">
